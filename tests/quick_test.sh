#!/bin/bash
# 🧪 Quick Test Script # Generate unique username for each test run
TIMESTAMP=$(date +%s)
...
TOKEN=$(curl -s -X POST "$BASE_URL/api/login" -H "Content-Type: application/json" -d '{"username":"'$TEST_USER'","password":"Test123!"}' | python3 -c "import sys, json; print(json.load(sys.stdin)['token'])" 2>/dev/null || echo "")sential Tests Only
...
# Essential tests
quick_test "Server Health" "curl -s $BASE_URL/api/status | grep -q healthy"
quick_test "User Registration" "curl -s -X POST $BASE_URL/api/register -H 'Content-Type: application/json' -d '{\"username\":\"quicktest\",\"email\":\"quick@test.com\",\"password\":\"Test123!\"}' | grep -q successfully"
quick_test "User Login" "curl -s -X POST $BASE_URL/api/login -H 'Content-Type: application/json' -d '{\"username\":\"quicktest\",\"password\":\"Test123!\"}' | grep -q token"

# Get token for authenticated tests
TOKEN=$(curl -s -X POST "$BASE_URL/api/login" -H "Content-Type: application/json" -d '{"username":"quicktest","password":"Test123!"}' | python3 -c "import sys, json; print(json.load(sys.stdin)['token'])" 2>/dev/null || echo "")
...
if [ -n "$TOKEN" ]; then
    ...
    CONNECT_RESULT=$(curl -s -X POST $BASE_URL/api/accounts/1/connect -H 'Authorization: Bearer $TOKEN' -H 'Content-Type: application/json')
